...
func TestClientCreateLaptop(t *testing.T) {
	...
	// check that the saved laptop is the same ac the one we send
...
func TestClientUploadImage(t *testing.T) {
	...
	saverImagePath := fmt.Sprintf("%s/%s%s", testImageFolder, res.GetId(), imageType)
	require.FileExists(t, saverImagePath)
	require.NoError(t, os.Remove(saverImagePath))
}
...
func newTestLaptopClient(t *testing.T, serverAdress string) pb.LaptopServiceClient {
	conn, err := grpc.Dial(serverAdress, grpc.WithTransportCredentials(insecure.NewCredentials()))
	require.NoError(t, err)
